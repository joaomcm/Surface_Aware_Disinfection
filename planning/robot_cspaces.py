...
class Robot3DCSpace(CSpace):
    ...
    def make_valid_config(self,cfig):
        cfig = np.array(cfig)
        cfig[self.fixed_dofs] = self.zero_cfig[self.fixed_dofs]
        return cfig.tolist()
    ...
    def select_disjoint_to_sample(self):
        ...
        # print('focus neighbor = {}'.format(focus_neighbor))
        nearest = np.argmin(distances)
        nearest_component = focus_nodes[nearest]
        nearest_connected = focus_neighbor[nearest][0]
        # distances,nearest_connected = self.clf.kneighbors(np.array(self.G_list[0])[focus_node,self.robot.numLinks():].reshape(1,-1))
        # print(distances,nearest_connected)

        # nearest_connected = nearest_connected[0][0]
        return self.G_list[0][nearest_component],np.array(self.G_list[0])[self.main_nodes][nearest_connected]
    ...
    def interp(self,m_a,m_b,steps = 25):
        ...
        # pdb.set_trace()
        for u in divs:
            interm_steps.append(self.robot.interpolate(m_a,m_b,u))
    ...
    def sample_nearby(self,milestone,angle_neighborhood = np.pi/12,spatial_neighborhood = 0.1,debug = False):
        ...
        for i in range(tries):
            ...
            # print('sample nearby  = {}'.format(np.array(cfig)[[2,4,5,10,11,12,13]]))
            self.robot.setConfig(cfig)
            if(debug):
                print(cfig)
            if(self.collisionChecker()):
        #                     valid = True
                ...
                new_cfig = self.make_valid_config(new_cfig)
                # new_cfig[9] = 0
    ...
    def sample(self):
        ...
        if(self.fraction > 0.3):
            if(np.random.rand() < explore):
                ...
                while(not valid):
                    ...
                    if(self.collisionChecker()):
                        ...
                        # print('random sample: {}'.format(np.array(new_cfig)[[2,4,5,10,11,12,13]]))
                        # we then get the cartesian position:
                        xyz = self.lamp.getWorldPosition(self.local_lamp_coords)
                        if(debug):
            ...
            # otherwise, we sample close to one of the remaining milestones
                ...
                for i in self.linear_dofs:
                    this_min[i] = np.clip(milestone[i]-neighborhood,this_min[i],this_max[i])
                    # this_min[1] = milestone[1]-neighborhood
                    this_max[i] = np.clip(milestone[i]+neighborhood,this_min[i],this_max[i])
                    # this_max[1] = milestone[1]+neighborhood
                #             print(this_min[:2],this_max[:2])
                valid = False
                for i in range(tries):
                    cfig = np.random.uniform(this_min[:self.robot.numLinks()],this_max[:self.robot.numLinks()])
                    cfig = self.make_valid_config(cfig)
                    # print('random sample near config: {}'.format(np.array(cfig)[[2,4,5,10,11,12,13]]))
                    # print(len(cfig))
                    self.robot.setConfig(cfig)
                    if(debug):
                        print(cfig)
        ...
        #otherwise, perform agressive sampling near the not connected components
            ...
            if(self.counter%10 == 0):
                ...
                self.this_focus = focus
                self.this_nearest_connected = nearest_connected
                self.counter = 1
            else:
                focus = self.this_focus
                nearest_connected = self.this_nearest_connected
                self.counter +=1
...
class CSpaceObstacleSolver:
        def __init__(self,space,milestones = (0,0),initial_points = 4000,steps = 100):
            self.space = space
            #PRM planner
            MotionPlan.setOptions(type="prm",knn=20,connectionThreshold=0.5,ignoreConnectedComponents = True)
        ...
        def get_adjacency_matrix_from_milestones(self):
            ...
            while(self.connected == False):
                ...
                print('Remaining to connect: {}'.format(len(self.milestones) - len(self.connected_list)))
